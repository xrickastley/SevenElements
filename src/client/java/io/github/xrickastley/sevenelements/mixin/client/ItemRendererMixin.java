...
public class ItemRendererMixin {
	@WrapOperation(
		...
		at = @At(
			value = "INVOKE",
			target = "Lnet/minecraft/client/render/item/ItemRenderer;getSpecialItemGlintConsumer(Lnet/minecraft/client/render/VertexConsumerProvider;Lnet/minecraft/client/render/RenderLayer;Lnet/minecraft/client/util/math/MatrixStack$Entry;)Lnet/minecraft/client/render/VertexConsumer;"
		)
	)
	private static VertexConsumer renderAttunementGlint$1(VertexConsumerProvider provider, RenderLayer layer, MatrixStack.Entry entry, Operation<VertexConsumer> original, @Local(argsOnly = true) ItemRenderState.Glint glintState) {
		return ElementGlintRenderer.getSpecialDisplayGlintConsumer(original.call(provider, layer, entry), provider, layer, entry, glintState);
	}
