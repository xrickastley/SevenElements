	...
	implements PayloadHandler<ShowElectroChargeS2CPayload>
{
	...
	protected void render(WorldRenderContext context, ChargeEffectState state) {
		...
		final BufferBuilder lineBuffer = this.getBuffer(SevenElementsRenderPipelines.CHARGE_LINE, RenderLayer.field_64009);
		...
		chargePositions
			...
			.forEach(positions -> {
				...
				state.positions = positions.getPositions()
					.stream()
					.map(pos -> pos.add(positions.initialPos).add(livingEntity.getEntityPos()))
					.toList();
