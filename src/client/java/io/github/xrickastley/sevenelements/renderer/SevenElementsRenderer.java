		...
		try {
			...
			try (
				RenderPass renderPass = RenderSystem.getDevice()
					...
					.createRenderPass(
						Functions.supplier("Draw for SevenElementsRenderer instance: " + this.getClass().getSimpleName() + " with RenderPipeline: " + pipeline.getLocation()), framebuffer.getColorAttachmentView(), OptionalInt.empty(), framebuffer.getDepthAttachmentView(), OptionalDouble.empty()
					)
			) {
				...
				if (texture.isPresent())
					renderPass.bindTexture("Sampler0", client.getTextureManager().getTexture(texture.get()).getGlTextureView(), null);
