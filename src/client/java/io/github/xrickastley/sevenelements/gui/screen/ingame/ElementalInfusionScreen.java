...
public class ElementalInfusionScreen extends HandledScreen<ElementalInfusionScreenHandler> {
	...
	private void drawElement(final Element element, final DrawContext context, final int x, final int y) {
		...
		context.drawTexture(RenderPipelines.GUI_TEXTURED, element.getTexture(), x, y, 0, 0, 24, 24, 24, 24, color);
	}
	...
	private void drawInfuseButton(DrawContext context, final int x, final int y, final int mouseX, final int mouseY) {
		...
		context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, texture, x1, y1, width, height);
	...
	private void drawUninfuseButton(DrawContext context, final int x, final int y, final int mouseX, final int mouseY) {
		...
		context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, texture, x1, y1, width, height);
		context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, expTexture, x2 - 18, y2 - 16, 16, 16);
	...
	private boolean checkMouseClickUninfused(Click click, boolean doubled) {
	...
	private boolean checkMouseClickInfused(Click click, boolean doubled) {
	...
	private boolean isInRectangle(Click click, double x, double y, double dx, double dy) {
