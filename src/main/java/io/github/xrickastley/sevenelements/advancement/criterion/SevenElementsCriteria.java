...
public class SevenElementsCriteria {
	...
	public static void register() {
		register(ElementalInfusionCriterion.ID, SevenElementsCriteria.ELEMENTAL_INFUSION);
		register(PerformAttunementCriterion.ID, SevenElementsCriteria.PERFORM_ATTUNEMENT);
		register(ReactionTriggeredCriterion.ID, SevenElementsCriteria.REACTION_TRIGGERED);
	}

	private static void register(Identifier id, Criterion<?> criterion) {
		Criteria.register(id.toString(), criterion);
	}
