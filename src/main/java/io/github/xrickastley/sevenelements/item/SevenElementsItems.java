...
public class SevenElementsItems {
	public static final Item INFUSION_TABLE = register(SevenElementsBlocks.INFUSION_TABLE, DoubleHighBlockItem::new);
	...
	public static void register() {
		...
		ModifyEntryHandler.addAfter(CreativeModeTabs.FUNCTIONAL_BLOCKS, Items.CRAFTING_TABLE, SevenElementsItems.INFUSION_TABLE);
		ModifyEntryHandler.addAllAfter(CreativeModeTabs.INGREDIENTS, Items.BOLT_ARMOR_TRIM_SMITHING_TEMPLATE,
	...
	public static Item register(Block block, BiFunction<Block, Item.Properties, Item> factory) {
		return register(block.builtInRegistryHolder().key().identifier(), settings -> factory.apply(block, settings), UnaryOperator.identity());
	}
	...
	public static Item register(String id, Function<Item.Properties, Item> factory, UnaryOperator<Item.Properties> settingsOperator) {
		return register(SevenElements.identifier(id), factory, settingsOperator);
	}

	public static Item register(Identifier id, Function<Item.Properties, Item> factory, UnaryOperator<Item.Properties> settingsOperator) {
		final ResourceKey<Item> key = ResourceKey.create(Registries.ITEM, id);

		return register(
			key,
			factory.apply(settingsOperator.apply(new Item.Properties().setId(key)))
		);
	}

	public static Item register(ResourceKey<Item> key, Item item) {
	...
	public static class ModifyEntryHandler implements ModifyOutputAll {
		...
		public static void prependAll(ResourceKey<CreativeModeTab> group, Item... items) {
		...
		public static void addAll(ResourceKey<CreativeModeTab> group, Item... items) {
			ENTRIES.get(group).add(new Entry(List.of(items), EntryType.ADD, null));
		}

		public static void addBefore(ResourceKey<CreativeModeTab> group, Item before, Item item) {
			ENTRIES.get(group).add(new Entry(item, EntryType.ADD_BEFORE, before));
		}

		public static void addAllBefore(ResourceKey<CreativeModeTab> group, Item before, Item... items) {
			ENTRIES.get(group).add(new Entry(List.of(items), EntryType.ADD_BEFORE, before));
		}

		public static void addAfter(ResourceKey<CreativeModeTab> group, Item after, Item item) {
			ENTRIES.get(group).add(new Entry(item, EntryType.ADD_AFTER, after));
		}

		public static void addAllAfter(ResourceKey<CreativeModeTab> group, Item after, Item... items) {
	...
	private static enum EntryType {
		...
		ADD {
			@Override
			void add(Entry entry, FabricCreativeModeTabOutput entries) {
				entries.acceptAll(entry.getItems());
			}
		},
		ADD_AFTER {
			@Override
			void add(Entry entry, FabricCreativeModeTabOutput entries) {
				entries.insertAfter(entry.relativeItem, entry.getItems());
			}
		},
		ADD_BEFORE {
			@Override
			void add(Entry entry, FabricCreativeModeTabOutput entries) {
				entries.insertBefore(entry.relativeItem, entry.getItems());
			}
		};
