...
public class ElementalAttunementSmithingTemplateItem extends Item {
	...
	ElementalAttunementSmithingTemplateItem(Item.Settings settings) {
		super(settings);
	}
	...
	private void playerInventoryTick(ItemStack stack, World world, PlayerEntity entity) {
		this.tickAttunementMethod(
			...
			player -> player.getEntityWorld().getDimensionEntry().matchesKey(DimensionTypes.THE_NETHER) && player.sevenelements$isFullySubmergedIn(FluidTags.LAVA)
		);
		this.tickAttunementMethod(
			entity,
		...
		this.tickAttunementMethod(
			...
			player -> player.isInsideWall() && player.getEyeY() <= 0 && player.getEntityWorld().getBlockState(BlockPos.ofFloored(player.getEyePos())).isIn(SevenElementsBlockTags.PROGRESSES_GEO_ATTUNEMENT)
		);
	}
	...
	public static record AttunementPityCounterReference(Element attunement, Identifier counter, double baseChance, int pityStart, double chancePerPity, Supplier<ItemStack> reward) {
		public boolean roll(PlayerEntity player) {
			...
			if (component.getOwner().getEntityWorld().getRandom().nextDouble() < component.getChanceFromPityCounter(counter, baseChance / 100, pityStart, chancePerPity / 100)) {
