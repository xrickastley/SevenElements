...
public class ElementalAttunementSmithingTemplateItem extends Item {
	private static final Map<Identifier, LootPool.Builder> MODIFIED_LOOT_POOLS = new HashMap<>();
	...
	private static void registerLootTableModification(Identifier lootTableKey, LootPool.Builder lootPool) {
	...
	static void registerLootTableModifications() {
		...
		LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
			if (source.isBuiltin() && MODIFIED_LOOT_POOLS.containsKey(id))
				tableBuilder.pool(MODIFIED_LOOT_POOLS.get(id));
		});
	}
